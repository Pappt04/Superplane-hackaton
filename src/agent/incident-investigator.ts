...
function generatePostmortem(
  ...
  const status = fixApplied ? "RESOLVED (automated)" : "ESCALATED — requires human intervention";
  const mttrStr = mttrSeconds < 60
    ? `${Math.round(mttrSeconds)}s`
    : `${Math.floor(mttrSeconds / 60)}m ${Math.round(mttrSeconds % 60)}s`;
  const actionsText = actionsTaken.length > 0
    ? actionsTaken.slice(0, 6).map((a, i) => `${i + 1}. ${a.split("->")[0].trim()}`).join("\n")
    : "No remediation actions attempted";
  return [
    `## Incident Report — ${alert.service}`,
    ``,
    `**Status:** ${status}`,
    ...
    actionsText,
    ``,
    fixApplied ? "" : `### Proposed Fix\n${proposedFix}\n`,
    `### Resolution`,
    fixApplied
      ? `Service automatically restored by AI On-Call in **${mttrStr}**. Zero human intervention required.`
      : `AI could not resolve automatically. Manual intervention required. See proposed fix above.`,
    ``,
    `---`,
    `*Generated by AI On-Call Engineer powered by SuperPlane*`,
  ].join("\n");
}
...
`.trim(),
  ...
  const mttrSeconds = (Date.now() - startTime) / 1000;
  const rootCause = rootCauseMatch?.[1]?.trim() || lastText.slice(0, 800);
  const proposedFix = fixApplied ? "" : (proposedFixMatch?.[1]?.trim() || lastText.slice(0, 900));
  ...
  return {
    ...
    postmortem: generatePostmortem(alert, rootCause, actionsTaken, fixApplied, proposedFix, mttrSeconds),
